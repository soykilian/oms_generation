...
kernels = {
    ...
    #"neuroscience": (gaussian_filter, neuro_activation_function, 1, .9),
}
    ...
    for i, (data, targets, file_name, class_name) in enumerate(iter(loader)):
        ...
        if False:
            ...
            oms(
                ...
                save_dir=os.path.join(save_dir, 'results/', name[0] + f"_{kernel}"),
                dvs_frames=data,
                compile_video=True,
        ...
        file_path = os.path.join(save_dir, file_name[0] + f"_{kernel}")
        print(file_path)
        # run OMS
        oms_frames = oms(
            params_dir=params_file + '.yaml',
